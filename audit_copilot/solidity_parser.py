...
def _remove_function_bodies(contract_body: str) -> str:
    """Remove function bodies before scanning top-level state variables.

    The parser is intentionally heuristic, but state variable extraction should
    not treat local variables declared inside functions as contract storage.
    """
    cleaned = []
    cursor = 0

    for match in FUNCTION_RE.finditer(contract_body):
        cleaned.append(contract_body[cursor:match.start()])

        function_open = contract_body.find("{", match.end() - 1)
        if function_open == -1:
            cursor = match.end()
            continue

        function_end = find_matching_brace(contract_body, function_open)
        cursor = function_end + 1

    cleaned.append(contract_body[cursor:])
    return "\n".join(cleaned)
...
def parse_solidity_file(path: Path, root: Path) -> list[ContractInfo]:
    ...
    for match in CONTRACT_RE.finditer(source):
        ...
        state_vars = []
        state_scan_body = _remove_function_bodies(body)

        for sv in STATE_VAR_RE.finditer(state_scan_body):
        ...
        for fm in FUNCTION_RE.finditer(body):
            ...
            modifiers = [
                t
                for t in tokens
                if t not in VISIBILITIES
                and t not in {"payable", "view", "pure", "virtual", "override", "returns"}
            ]
